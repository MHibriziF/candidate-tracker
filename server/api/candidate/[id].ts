...
export default defineEventHandler(async (event) => {
  ...
  try {
    if (method === 'GET') {
      ...
      if (!candidate) throw Errors.notFound(`Candidate with id ${id} not found`);
      return candidate;
    }

    if (method === 'PATCH') {
      const body = await readBody<{ status: CandidateStatus }>(event);
      if (!body?.status) throw Errors.badRequest('Status is required for update');
      if (!Object.values(CandidateStatus).includes(body.status as CandidateStatus)) {
